...
class WechatPay {
    ...
    const URL_O2_AUTHORZE     = 'https://open.weixin.qq.com/connect/oauth2/authorize'; // Wing
    const URL_O2_ACCESS_TOKEN = 'https://api.weixin.qq.com/sns/oauth2/access_token'; // Wing
    const URL_SNS_USERINFO    = 'https://api.weixin.qq.com/sns/userinfo'; //Wing
    /**
     * 错误信息
     */
    public $error = null;
    ...
    /**
     * Wing!!!! jsapi支付的第一步, 构建静默授权的链接
     */
    public function jsapiPayStep1Url($redirect_uri){

		//微信提供state接口携带参数, 非必须, 与获取用户信息不一样的是scode为snsapi_base, 静默授权, 只为获取openid
		$url = self::URL_O2_AUTHORZE."?appid=".$this->_config['appid']."&redirect_uri=".$redirect_uri."&response_type=code&scope=snsapi_base#wechat_redirect";
    ...
    /**
     * Wing!!!! jsapi支付的第二步, 获取预支付ID
     */
    public function getOpenId($code){

		// 第2步 通过code获取openid
		$get_openid_url = self::URL_O2_ACCESS_TOKEN.'?appid='.$this->_config['appid'].'&secret='.$this->_config['appsecret'].'&code='.$code.'&grant_type=authorization_code';
		$arr = json_decode($this->httpGet($get_openid_url), true);
        // return $arr;
        return $arr['openid'];
    ...
    /**
     * Wing!!!! 获取用户信息的第一步, 构建点击授权的链接
     */
    public function userInfoStep1Url($redirect_uri){
        $url = self::URL_O2_AUTHORZE.'?appid='.$this->_config['appid']."&redirect_uri=".$redirect_uri."&response_type=code&scope=snsapi_userinfo#wechat_redirect";
        return $url;
    }
    ...
    public function getUserInfo($code)
    {
        ...
        // 第2步 通过code获取网页授权access_token和openid
        $get_token_url = self::URL_O2_ACCESS_TOKEN.'?appid='.$this->_config['appid'].'&secret='.$this->_config['appsecret'].'&code='.$code.'&grant_type=authorization_code';
//        return $get_token_url;
        $json_obj = json_decode($this->httpGet($get_token_url), true);
//        return $json_obj;

        // 第3步, 根据openid和access_token查询用户信息
        $access_token = $json_obj['access_token'];
        $openid = $json_obj['openid'];

        $get_user_info_url = self::URL_SNS_USERINFO.'?access_token='.$access_token.'&openid='.$openid.'&lang=zh_CN';
        $user_obj = json_decode($this->httpGet($get_user_info_url), true);
